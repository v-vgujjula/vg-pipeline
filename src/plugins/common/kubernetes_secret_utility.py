...
def get_kubernetes_secret(api_instance, namespace, secret_name):
    try:
        ...
        pytest.fail("Error occurred when retrieving secret '{}': ".format(secret_name) + str(e))
...
def watch_kubernetes_secret(api_instance, namespace, secret_name, timeout, callback=None):
    ...
    try:
        ...
        pytest.fail("Error occurred when watching kubernetes secret events: " + str(e))
    pytest.fail("The watch on the kubernetes secret events has timed out. Please see the pod logs for more info.")
