...
def get_source_control_configuration_client(credential, subscription_id, base_url=None, credential_scopes=None):
    try:
        return get_kubernetes_configuration_client(credential, subscription_id, base_url=base_url, credential_scopes=credential_scopes).source_control_configurations
...
def create_kubernetes_configuration(kc_client, resource_group_name, repository_url, cluster_rp, cluster_type, cluster_name,
    ...
    try:
        ...
        pytest.fail("Error occured while creating the kubernetes configuration resource: " + str(e))
...
def show_kubernetes_configuration(kc_client, resource_group_name, cluster_rp, cluster_type, cluster_name, configuration_name):
    try:
        ...
        pytest.fail("Error occured while fetching the kubernetes configuration resource: " + str(e))

def delete_kubernetes_configuration(kc_client, resource_group_name, cluster_rp, cluster_type, cluster_name, configuration_name): 
    return kc_client.begin_delete(resource_group_name, cluster_rp, cluster_type, cluster_name, configuration_name, polling=False)