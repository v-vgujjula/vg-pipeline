...
def env_dict():
    ...
    with FileLock(str(my_file) + ".lock"):  # Locking the file since each test will be run in parallel as separate subprocesses and may try to access the file simultaneously.
        ...
        if not my_file.is_file():
            ...
            env_dict['RELEASE_TYPE'] = os.getenv('RELEASE_TYPE')
            env_dict['REPOSITORY'] = os.getenv('REPOSITORY')
            env_dict['IMAGE_TAG'] = os.getenv('IMAGE_TAG')
            env_dict['DS_EXTN_VER_TAG'] = os.getenv('DS_EXTN_VER_TAG')
            env_dict['DS_EXTN_REL_TRAIN'] = os.getenv('DS_EXTN_REL_TRAIN')
