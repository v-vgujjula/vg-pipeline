    ...
    try:
        ...
        try:
            ...
            while True:
                resource_list = resource_client.resources.list_by_resource_group(resource_group, expand = "createdTime,changedTime")
                for resource in list(resource_list):
                    if resource.type == ds_connect_constants.SQL_MI_TYPE and  resource.name == env_dict.get('SQL_INSTANCE_NAME'):
                        time.sleep(sleep_time)
        ...
        while True:
            resource_list = resource_client.resources.list_by_resource_group(resource_group, expand = "createdTime,changedTime")
            for resource in list(resource_list):
                if resource.type == ds_connect_constants.DATA_CONTROLLER_TYPE and  resource.name == "arc-ds-controller":
                    time.sleep(sleep_time)
                    timeout_interval=timeout_interval+1
                    dc_status = True
                    break
